...
class PhilipsGatewayPowerSwitch(SwitchEntity):
    ...
    async def _async_set_power(self, enabled):
        _LOGGER.warning("gateway power switch: setting esn=%s enabled=%s", self._esn, enabled)
        resp = await self.hass.async_add_executor_job(
            self._api.set_gateway_power_status,
            self._esn,
            enabled,
        )
        _LOGGER.warning("gateway power switch: set response for esn=%s: %s", self._esn, resp)
